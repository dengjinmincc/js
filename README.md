...
1. 什么是闭包
	...
	> 简单来说，闭包就是在另一个作用域中保存了一份它从上一级函数或> 作用域取得的变量（键值对），而这些键值对是不会随上一级函数的> 执行完成而销毁。周爱民说得更清楚，闭包就是“属性表”，闭包就是> 一个数据块，闭包就是一个存放着“Name=Value”的对照表。就这么> > 简单。但是，必须要强调，闭包是一个运行期概念。
